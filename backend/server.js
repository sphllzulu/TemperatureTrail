...
app.use(
  cors({
    origin: ['https://temperaturetrail-2.onrender.com', 'http://localhost:5173'],
    credentials: true,
  })
);
app.use(express.json());
...
app.get('/api/activities', async (req, res) => {
  ...
  try {
    const weatherResponse = await axios.get(`${BASE_URL}/weather?q=${city}&appid=${WEATHER_API_KEY}`);
    const { lat, lon } = weatherResponse.data.coord;
    ...
    switch (weatherCondition.toLowerCase()) {
      case 'sunny':
      case 'clear':
        categoryIds = '16032,16000,16015';
        break;
      case 'rain':
      case 'clouds':
        categoryIds = '10025,13003,10035';
        break;
      case 'snow':
        categoryIds = '16026,10056';
        break;
      default:
        categoryIds = '16000,13065';
    }
...
// Search History and Favorites Routes (Protected by authMiddleware)
app.get('/api/search-history', authMiddleware, async (req, res) => {
  const userId = req.session.userId;
  try {
    const user = await User.findById(userId);
